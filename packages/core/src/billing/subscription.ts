...
export function paymentRequestMessage(paymentUrl: string, amountIdr: number, periodDays: number): string {
  return (
    `Satu langkah lagi! 🧾 Untuk mengaktifkan layanan ${periodDays} hari ke depan, ` +
    `selesaikan pembayaran ${rupiah(amountIdr)} di sini:\n${paymentUrl}\n\n` +
    'Setelah pembayaran diterima, layananmu langsung aktif dan kukabari di sini ya.'
  );
}
...
export async function pollPendingInvoices(deps: PollDeps): Promise<Result<PollResult, Error>> {
  ...
  for (const inv of pending.value) {
    ...
    if (st.value === 'PAID') {
      ...
      await deps.invoices.markStatus(inv.id, st.value);
      expired += 1;
    }
  }
